...
class GlobalExceptionHandlerTest {
    ...
    void handleNotFoundDebeRetornar404() {
        ProblemDetail problem = handler.handleNotFound(new EmpleadoNotFoundException("EMP-1"));
        assertThat(problem.getStatus()).isEqualTo(HttpStatus.NOT_FOUND.value());
    }

    @Test
    void handleClaveDebeRetornar500() {
        ProblemDetail problem = handler.handleClave(new ClaveGeneracionException("error"));
